  ...
  const filteredCourses = useMemo(() => {
    return microCourses.filter((course) => {
      const taskMatch =
        ...
        course.tags.tasks.some((t) => t.includes(selectedTask.split(" ")[0]));
      const subjectMatch =
        selectedSubject === "All Subjects" ||
        course.tags.subjects.includes(selectedSubject) ||
        course.tags.subjects.includes("Universal");
      ...
      <Layout>
        <div className="container mx-auto px-4 py-8">
          <ConstraintsCourse
            ...
            onCompleteSection={(sectionId) => completeSection(activeCourse, sectionId)}
            onAnswerCFU={(cfuId, selected, correct) => answerCFU(activeCourse, cfuId, selected, correct)}
          ...
          <CardContent className="pt-6">
            <div className="max-w-3xl">
              <h2 className="text-xl font-semibold text-foreground mb-3">
                This is not training. This is productive work time.
              </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCourses.map((course) => (
            <MicroCourseCard
              ...
              onClick={() => handleCourseClick(course.id, course.isPlaceholder)}
            />
          ))}
        </div>
      </section>
